...
namespace Smart3dMap {
    ...
    */
	class _ThreadExport ResourceManager : public ResourceAlloc
    {
    public:
        ...
        */
        virtual ResourcePtr createResource(const OC::String& name, const OC::String& group, 
		...
		*/
		virtual ResourceCreateOrRetrieveResult createOrRetrieve(const OC::String& name, 
			const OC::String& group, bool isManual = false, 
		...
		*/
		virtual void unload(const OC::String& name);
		...
		*/
        ...
        @note
            ...
            shared pointers before you shutdown OGRE.
		*/
		virtual void remove(const OC::String& name);
        ...
        */
        virtual ResourcePtr getResourceByName(const OC::String& name, const OC::String& groupName = ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME);
        /** Retrieves a pointer to a resource by handle, or null if the resource does not exist.
        */
        virtual ResourcePtr getByHandle(ResourceHandle handle);
		
		/// Returns whether the named resource exists in this manager
		virtual bool resourceExists(const OC::String& name)
		{
		...
		*/
		virtual ResourcePtr prepare(const OC::String& name, 
            const OC::String& group, bool isManual = false, 
		...
		@param backgroundThread Optional boolean which lets the load routine know if it
			is being run on the background resource loading thread
		*/
		virtual ResourcePtr load(const OC::String& name, 
            const OC::String& group, bool isManual = false, 
			ManualResourceLoader* loader = 0, const NameValuePairList* loadParams = 0,
			bool backgroundThread = false);
		...
		*/
		virtual const OC::StringVector& getScriptPatterns(void) const { return mScriptPatterns; }
		...
		@param groupName The name of the resource group that resources which are
			...
			with it.
		*/
		virtual void parseScript(DataStreamPtr& stream, const OC::String& groupName)
		...
		*/
		...
		/** Gets a OC::String identifying the type of resource this manager handles. */
		const OC::String& getResourceType(void) const { return mResourceType; }
		...
		*/
		class _ThreadExport ResourcePool : public Pool<ResourcePtr>, public ResourceAlloc
		{
		protected:
			OC::String mName;
		public:
			ResourcePool(const OC::String& name);
			~ResourcePool();
		...
		ResourcePool* getResourcePool(const OC::String& name);
		/// Destroy a resource pool
		void destroyResourcePool(ResourcePool* pool);
		/// Destroy a resource pool
		void destroyResourcePool(const OC::String& name);
		/// destroy all pools
    ...
    public:
		typedef HashMap< OC::String, ResourcePtr > ResourceMap;
		typedef HashMap< OC::String, ResourceMap > ResourceWithGroupMap;
    ...
    protected:
		...
		OC::StringVector mScriptPatterns; 
		/// Loading order relative to other managers, higher is later
		Real mLoadOrder; 
		/// OC::String identifying the resource type this manager handles
		OC::String mResourceType; 
	...
	protected:
		typedef std::map<OC::String, ResourcePool*> ResourcePoolMap;
