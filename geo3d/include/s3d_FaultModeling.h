...
namespace Smart3dMap
{
	...
	public:
		...
		int   CreateFaultSurfaces2(vector<s3d_FaultLine>  &Fault, double depth, int num,
			QMap<string, vector<vector<int>>>             &FaultNos,
			QMap<string, vector<vector<gme_vector3d>>>    &FaultDot,
			QMap<string, vector<s3d_FaultLine>>           &MapFault,
			QMap<string, vector<s3d_Pointer<gmeSurface>>> &outSurfs);
	...
	public:
		...
		// 利用断裂编号对断裂线进行重组
		int   FaultGroupByNo(vector<s3d_FaultLine> &FaultVec, QMap<string, vector<s3d_FaultLine>> &MapFault);
	};
	...
	public:
		...
		int  CreateInitialModels(vector<gmeSurface*>   &TopSurfs,
			...
			QMap<string, vector<vector<gmeSurface*>>> &FaultSurfs);
	...
	private:
		...
		void ReleaseDatas(QMap<FAULT_INNER*, gmeSurface*> &vec);
		void ReleaseDatas(QMap<string, vector<vector<gmeSurface*>>> &vec);
	...
	public:
		...
		QMap<FAULT_INNER*, gmeSurface*>         m_faultSurf; //辅断层对应的延伸面 析构时清空
		QMap<FAULT_INNER*, FAULT_INNER*>           m_faultMap1; //辅断层对应的主断层...
		QMap<FAULT_INNER*, vector<FAULT_INNER*>>   m_faultMap2; //主断层对应的辅断层...

	public:
		QMap<gmeSurface*, FAULT_INNER>                m_SurfFlag;  //侧面、断面对应的信息
		QMap<gmeSurface*, vector<gme_vector3d>>    m_SurfBdary; //顶面、侧面、断面对应的边界线数据...
		QMap<string, vector<gme_rect_2d>>          m_Rect2D;    //每个断层编号 对应的断层数据...
		QMap<string, vector<vector<int>>>          m_pFaultNos; //每个断层编号 对应的断层数据...
		QMap<string, vector<vector<gme_vector3d>>> m_pFaultDot; //每个断层编号 对应的断层数据...
		QMap<string, vector<s3d_FaultLine>>        m_pMapFault; //每个断层编号 对应的断层数据...

		QMap<string, vector<s3d_Pointer<gmeSurface>>> m_pOutSurfs;  //每个断层编号   对应的断层数据...
		QMap<string, vector<vector<gmeSurface*>>>     m_pLinkSurfs; //m_pLinkSurfs 剪切后的所有面\需要在外面释放
		QMap<string, vector<vector<gme_rect_2d>>>     m_pLinkRects; //m_pLinkSurfs 剪切后的所有面
	...
	public:
		...
		int  CreateStratEntities(QMap<string, vector<EnyRef>> &OutEntitiesMap, vector<EnyRef> *pFaultCutBox);
		...
		int  CreateStratumModel(vector<gmeEntity*>       &FrameEntyVec,    // 输入数据：断层框架
			...
			QMap<string, vector<EnyRef>> &OutEntitiesMap); //地层编码 对应的 地层
	...
	private:
		...
		int  CreateGeoModel(gmeEntity      * pFrame,
			gmeSurface     * pTmpSurf,
			vector<gme_vector3d> & bdyVec,
			QMap<string, vector<EnyRef>> & EnyVec);
	...
	public:
		...
		QMap<gmeEntity*, gmeEntity*>              m_MapEntyToEnty;   // 对应的裁剪体
		QMap<gmeEntity*, gme_rect_3d*>            m_MapEntyToRect;   // 对应的DRect
		QMap<gmeEntity*, s3d_Polygon*>            m_MapEntyToPoly;   // 对应的范围
		QMap<gmeEntity*, vector<s3d_GeoPolygon>>  m_MapEntyToRes;    // 对应的约束多边形
		QMap<gmeEntity*, vector<s3d_BoreHole*>>   m_MapEntyToHole;   // 对应的钻孔
	public:
		...
		QMap<string, vector<EnyRef>>            m_OutEntitiesMap;  // 最终输出的地层体模型
