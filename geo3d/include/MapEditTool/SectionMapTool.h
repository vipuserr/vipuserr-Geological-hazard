...
namespace Smart3dMap {
	...
	/**
	 * 剖面图编辑工具。
	 * 该工具是参照 QgsVertexTool 实现的，增加的约束有：
	 * （1）钻孔分层点不可被修改（移动或删除）；
	 * （2）地层尖灭点（连接2条以上弧段的顶点）只可被移动，不能被删除；
	 * （3）修改地层区的弧段时，同步修改地质界线；
	 * （4）不依赖于全局QgsProject对象(QgsProject::instance())。
	 */
	class S3D_QGSMAPTOOL_EXPORT SectionMapTool : public QgsMapToolAdvancedDigitizing
	{
		Q_OBJECT
	public:
		...
		SectionMapTool(QgsMapCanvas *canvas, QgsAdvancedDigitizingDockWidget *cadDock, VertexToolMode mode = SectionMapTool::AllLayers, bool enableVertexPanel = false);

		//! Cleanup canvas items we have created
		~SectionMapTool() override;
	...
	private:
		...
		*/
		...
		void addExtraSegmentsToEdits(SectionMapTool::VertexEdits &edits, const QgsPointXY &mapPoint, QgsVectorLayer *dragLayer, const QgsPoint &layerPoint);
		...
		void updateFeatureBand(const QgsPointLocator::Match &m, bool resetHighlightedVertices = false);
		...
		// 查找钻孔轨迹线图层。
		QgsVectorLayer* findLayerOfHoleLine() const;

		using PinchoutPointList = QVector<QgsPointXY>;	// 剖面图中的地层尖灭点：连接2条以上弧段的顶点。
		void getPinchoutVerticesFromGeoReg(PinchoutPointList& points, QgsVectorLayer* layerReg = nullptr);
		// 查询坐标位置对应的尖灭点的位置。返回-1表示不存在对应的尖灭点。
		int findPinchoutVertexIndex(double x, double y, const PinchoutPointList& pinches) const;
		int findPointFromArray(double x, double y, const QVector<QgsPointXY>& pinches) const;

		bool isVertexSelectable(const QgsPointLocator::Match& m) const;
		bool isVertexSelectable(const QgsPoint& pt) const;
		bool isVertexSelectable(const QgsPointXY& pt) const;
		bool isVertexSelectable(const QgsVectorLayer* layer, const QgsFeatureId& featureId, int vertexIndex) const;
		bool isVertexDeletable(const Vertex& vertex, const PinchoutPointList& pinches) const;
		bool isPointOnHoleLine(double x, double y, QgsVectorLayer* layerOfHoleLine) const;
		bool isPointOnPolyline(double x, double y, const QgsPolylineXY& polyline) const;
		...
		// 对选中的弧段执行局部更新
		void smoothArcPartAtVertex(const Vertex& vertex);
		// 以指定顶点为中心，其左右各2各顶点一起组成的4个线段将被用于局部光滑。
		void findSegmentsAroundVertex(const QgsPolygonXY& polygon, const QgsPointXY& vertex, QVector<QgsPointXY>& ptSequence, const PinchoutPointList& pinches);
		// 对指定的线段执行过原始点的线段光滑。
		using SmoothCacheItem = std::pair<QVector<QgsPointXY>, QVector<QgsPointXY>>;
		using SmoothCache = std::list<SmoothCacheItem>;
		void smoothSegments(const QVector<QgsPointXY>& ptSequence, QVector<QgsPointXY>& smoothedSequence, SmoothCache& smoothCache);
		// 依据局部光滑的结果修改原数据（地层区），同时修改地质界线数据。返回true表示修改是有效的，否则表示无效，应该撤销当前操作。
		bool modifySegmentsInLayer(const QVector<QgsPointXY>& ptSequence, const QVector<QgsPointXY>& smoothedSequence, QgsVectorLayer* layerReg, const QgsFeatureId& regId, QgsVectorLayer* layerLin = nullptr);
		QgsAbstractGeometry* modifyGeometry(QgsGeometry* originalGeometry, const QVector<QgsPointXY>& ptSequence, const QVector<QgsPointXY>& smoothedSequence) const;
		bool isSamePosition(const QgsPointXY& a, const QgsPointXY& b) const;
		bool isSamePosition(const QgsPointXY& a, const QgsPoint& b) const;

		bool isGeometryValid(const QgsGeometry& geo) const;
		void tipOfInvalidEdit();
	...
	private:
		...
		{
			SectionMapTool *tool = nullptr;               //!< Pointer to the parent vertex tool (for connections / canvas)
			...
			void start(QgsGeometry &geom, SectionMapTool *tool, QgsVectorLayer *l);  //!< Start validation
		...
		bool mEnableVertexPanel;	// 是否显示顶点列表。在该列表上可直接编辑顶点的坐标，但其提示信息为英文。
		QgsProject* mQgsProject;	// 关联的QgsProject
	};
}
