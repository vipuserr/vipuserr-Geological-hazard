 ...
 * \ingroup gui
 *
 * Base class for all widgets shown on a QgsAttributeFormExt.
 * Consists of the widget which is visible in edit mode as well as the widget visible in search mode.
 *
 * \since QGIS 3.0
 */
class QgsAttributeFormWidgetExt : public QWidget // SIP_ABSTRACT
{
    Q_OBJECT
  ...
  public:
     ...
     */
    explicit QgsAttributeFormWidgetExt( QgsWidgetWrapper *widget, QgsAttributeFormExt *form );
     ...
     */
    QgsAttributeFormExt *form() const;
  ...
  private:
    ...
    QgsAttributeFormWidgetExt::Mode mMode = DefaultMode;
