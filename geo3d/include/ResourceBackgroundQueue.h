...
namespace Smart3dMap {
	...
	*/
	class _ThreadExport ResourceBackgroundQueue : public ResourceAlloc, 
	...
	protected:
		...
		struct ResourceRequest
		{
			...
			_ThreadExport friend std::ostream& operator<<(std::ostream& o, const ResourceRequest& r)
			{ (void)r; return o; }
		};
		...
		struct ResourceResponse
		{
			...
			_ThreadExport friend std::ostream& operator<<(std::ostream& o, const ResourceResponse& r)
			{ (void)r; return o; }
		};
	...
	public:
		...
		*/
		virtual void initialise(WorkQueue* wq);
		...
		*/
		virtual BackgroundProcessTicket initialiseResourceGroup(
			const OC::String& name, Listener* listener = 0);
		...
		*/
		...
		@return Ticket identifying the request, use isProcessComplete() to 
			determine if completed if not using listener
		*/
		virtual BackgroundProcessTicket prepareResourceGroup(const OC::String& name, 
			Listener* listener = 0);
		...
		@return Ticket identifying the request, use isProcessComplete() to 
			determine if completed if not using listener
		*/
		virtual BackgroundProcessTicket loadResourceGroup(const OC::String& name, 
			Listener* listener = 0);
		...
		*/
		virtual BackgroundProcessTicket unload(
			const OC::String& resType, const OC::String& name, 
			Listener* listener = 0);
		...
		*/
		virtual BackgroundProcessTicket unload(
			const OC::String& resType, ResourceHandle handle, 
			Listener* listener = 0);
		...
		@return Ticket identifying the request, use isProcessComplete() to 
			determine if completed if not using listener
		*/
		virtual BackgroundProcessTicket unloadResourceGroup(const OC::String& name, 
			Listener* listener = 0);
