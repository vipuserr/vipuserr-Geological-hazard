...
namespace Smart3dMap
{
	...
	public:
		...
		// 二维剖面图转三维剖面图
		static void section_2d_to_3d(gm_section_map& section_map);
		/* 三维剖面图转二维剖面图
		 * holes	: 入口，剖面图上的钻孔
		 * d3_dots	: 入口，待转换的三维点坐标
		 * d3_lines	: 入口，待转换的三维线坐标
		 * originX		: 剖面图x方向的起点,默认为0
		 * xScaleFactor	: x方向的缩放因子，默认为1，表示无缩放
		 * yScaleFactor	: y方向的缩放因子，默认为1，表示无缩放		 */
		static void section_3d_to_2d(vector<gm_section_hole>& holes, vector<gme_vector3d>& d3_dots, vector<gme_vector2d>& d2_dots,
			double originX = 0.0, double xScaleFactor = 1.0, double yScaleFactor = 1.0);
		static void section_3d_to_2d(vector<gm_section_hole>& holes, vector<vector<gme_vector3d>>& d3_lines, vector<vector<gme_vector2d>>& d2_lines,
			double originX = 0.0, double xScaleFactor = 1.0, double yScaleFactor = 1.0);
